...
procedure borrarEmp(var arch: fileEmpleados):

    input :  integer;
    arch_nuevo:fileEmpleados;
    reg:empleado;
    empaux:empleado;
    posBorrar:integer;
...
begin
    ...
    assign(arch_nuevo, 'archivo nuevo');
    rewrite(arch_nuevo);

    writeln('ingrese el numero de empleado que quiere borrar');
    readln(input);

    read(arch, reg);

    while not eof(arch)and (input<>reg.numEmpleado) do begin
        write(arch_nuevo, reg);
        read(arch, reg);
    end;

    //una vez lo encontre 
    read(arch, reg); //como ya lo tengo leido paso de largo y lo olvido.
    
    //copio lo que me queda
    ...
    rename(arch,'archempleados_old');
    rename(arch_nuevo,'archempleados');
end;
...
begin
    ...
    while input <> 0 do begin
        writeln('Presione 1 para agregar un empleado // Presione 2 para modificar la edad de un empleado // Presione 3 para borrar un Empleado.');
        readln(input);
